...
export default function Component() {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8 max-w-2xl mx-auto p-6 bg-white shadow-md rounded-lg">
            ...
            <div>
                <span className="block text-sm font-medium text-gray-700">Payment Methods</span>
                <p className="text-gray-500 text-sm mb-2">Select the payment methods you want to offer.</p>
                {paymentMethods.map((item) => (
                    <div key={item.id} className="flex items-start space-x-3 mt-2">
                        ...
                        />
                        <label htmlFor={`payment-${item.id}`} className="text-sm font-normal text-gray-700">{item.label}</label>
                    </div>
                ))}
                {errors.paymentMethods && <p className="text-red-600 text-sm mt-1">{errors.paymentMethods}</p>}
            </div>

            <div>
                <span className="block text-sm font-medium text-gray-700">Delivery Services</span>
                <p className="text-gray-500 text-sm mb-2">Select the delivery services you want to offer.</p>
                {deliveryServices.map((item) => (
                    <div key={item.id} className="flex items-start space-x-3 mt-2">
                        <input
                            ...
                            className="h-4 w-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500"
                        />
                        <label htmlFor={`delivery-${item.id}`} className="text-sm font-normal text-gray-700">{item.label}</label>
                    </div>
                ))}
                {errors.deliveryServices && <p className="text-red-600 text-sm mt-1">{errors.deliveryServices}</p>}
            </div>
